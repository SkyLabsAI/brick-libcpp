...
a fancy version of HeapLang, doesn't have opsem but does compile; logic evaluated with projects like TODO cite previous brick paper/projects

$\mathsf{current\_thread}$ (all predicates are actually monpreds on thread id) ...
Separation logic predicates are innately transferable unless they explicitly reference the thread that holds them. 
\input{specs.tex}
...
The \lstinline{lock_guard} class is the basic RAII class of the C++ library, a container for a single lock. \lstinline{lock_guard} is a templated class that takes an arbitrary \lstinline{BasicLockable} class as an argument, and our specification is likewise parameterized by a \lstinline{BasicLockable} instance. %not done yet
Constructing a \lstinline{lock_guard} with argument $m$ acquires $m$, and destroying it releases $m$: ...
...
The \lstinline{unique_lock} class offers a more sophisticated RAII interface: the lock can be manually acquired and released through the \lstinline{unique_lock} as well as at construction/destruction, and the \lstinline{unique_lock} can be detached from one lock and attached to another during its lifetime. ...
