...
Section with_cpp.
  ...
  cpp.spec "proper_subrange_results()" default.
  Lemma proper_subrange_results_ok :
    verify[test_cpp.source] "proper_subrange_results()".
  Proof using MOD.
    verify_spec; go.
    iExists 2%Z, (1$c)%cQp, [0; 0],
      (fun x => Some (bool_decide (x <> 0)%Z)), (fun _ => emp); go.
    rewrite (array_sliceR_app' (ty := Tuchar) cleared_payload_addr 0 1 4
      (fun x => ucharR (1$c)%cQp x) [9] [0; 0; 9]).
    1: rewrite (array_sliceR_app' (ty := Tuchar) cleared_payload_addr 1 3 4
         (fun x => ucharR (1$c)%cQp x) [0; 0] [9]).
    all: try lia.
    3: by vm_compute.
    2: by vm_compute.
    go.
    ...
    iExists 2%Z, (1$c)%cQp, [0; 0],
      (fun x => Some (bool_decide (x <> 0)%Z)), (fun _ => emp); go.
    iExists 2%Z, (1$c)%cQp, [4; 5],
      (fun x => Some (bool_decide (x <> 0)%Z)), (fun _ => emp); go.

    rewrite (array_sliceR_app' (ty := Tuchar) set_payload_addr 0 1 4
      (fun x => ucharR (1$c)%cQp x) [0] [4; 5; 0]).
    1: rewrite (array_sliceR_app' (ty := Tuchar) set_payload_addr 1 3 4
         (fun x => ucharR (1$c)%cQp x) [4; 5] [0]).
    all: try lia.
    3: by vm_compute.
    2: by vm_compute.
    go.
