
Require Import skylabs.auto.cpp.prelude.test.
Require Import skylabs.brick.libstdcpp.cassert.spec.
Require Import skylabs.brick.libstdcpp.optional.spec.
Require Import skylabs.brick.libstdcpp.test.optional.test_cpp.

Section with_cpp.
  Context `{Σ : cpp_logic} `{MOD : test_cpp.source ⊧ σ}.

  cpp.spec "reset_then_query_reports_false()" from test_cpp.source default.

  Lemma verify_reset_then_query_reports_false :
    verify[test_cpp.source] "reset_then_query_reports_false()".
  Proof.
    verify_spec.
    go $usenamed=true.

    (* Feed the engaged state to reset while retaining its type-derived reference fact. *)
    wname [ (value_addr |-> optionalR 1$m (Some 5)) ] "Hvalue".
    iDestruct (observe (type_ptr optionalT value_addr) with "Hvalue") as "#Htype".
    iDestruct (type_ptr_reference_to with "Htype") as "#Href".
    iExists (Some 5).
    iFrame "Hvalue Href".
    go $usenamed=true.

    iExists (1)%cQp, None.
    go $usenamed=true.
    iExists None.
    go $usenamed=true.
  Qed.
End with_cpp.
